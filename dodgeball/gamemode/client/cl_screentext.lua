...
local lastwait = false 
local firsttick = false 
// This shouldnt be here.... oh well!
local WMusic 
hook.Add("HUDPaint","Waitingforplayers",function()
	if firsttick == false then 
		WMusic = CreateSound(LocalPlayer(),"dodgeball/waiting_loop.wav")
		firsttick = true 
	end 
	if lastwait~=GAME.IsWaiting then 
		if GAME.IsWaiting==true then 

			WMusic:Play()
		end
		if GAME.IsWaiting==false then 
			WMusic:Stop()
		end
		lastwait = GAME.IsWaiting

	end

	if GAME.IsWaiting==true then 
			surface.SetDrawColor( 1, 0, 0, 200) --Red
			surface.DrawRect(0 , 0 , ScrW() , ScrH() )
			draw.SimpleText("Waiting for players...", "TEXREND", ScrW() / 2 , (ScrH() / 6) , Color(255,255,255,math.sin(CurTime()^1.144)*100 + 155), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			if #team.GetPlayers(TEAM_RED) > 0 and #team.GetPlayers(TEAM_BLUE) > 0 then 
					draw.SimpleText("Starting game in ~20 seconds...", "ChatFont", ScrW() / 2 , (ScrH() / 4) , Color(255,0,0,255), TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER)
			end 
